...
double polynomial_df_dw1(Coefficients c, Point points[], int n) {
	...
	for (int i = 0; i < n; ++i) {
		cumulative_sum += points[i].x * points[i].x * (polynomial_predict(c, points[i].x, points[i].y) - points[i].z);
	}
...
double polynomial_df_dw2(Coefficients c, Point points[], int n) {
	...
	double cumulative_sum = 0;
	for (int i = 0; i < n; ++i) {
		cumulative_sum += points[i].x * (polynomial_predict(c, points[i].x, points[i].y) - points[i].z);
	}
	return cumulative_sum / n;
...
double polynomial_df_dw3(Coefficients c, Point points[], int n) {
	...
	double cumulative_sum = 0;
	for (int i = 0; i < n; ++i) {
		cumulative_sum += points[i].y * points[i].y * (polynomial_predict(c, points[i].x, points[i].y) - points[i].z);
	}
	return cumulative_sum / n;	
}
...
double polynomial_df_dw4(Coefficients c, Point points[], int n) {
	...
	double cumulative_sum = 0;
	for (int i = 0; i < n; ++i) {
		cumulative_sum += points[i].y * (polynomial_predict(c, points[i].x, points[i].y) - points[i].z);
	}
	return cumulative_sum / n;	
}
...
double polynomial_df_dw5(Coefficients c, Point points[], int n) {
	...
	double cumulative_sum = 0;
	for (int i = 0; i < n; ++i) {
		cumulative_sum += points[i].x * points[i].y * (polynomial_predict(c, points[i].x, points[i].y) - points[i].z);
	}
	return cumulative_sum / n;	
}
...
double polynomial_df_dw6(Coefficients c, Point points[], int n) {
	...
	double cumulative_sum = 0;
	for (int i = 0; i < n; ++i) {
		cumulative_sum += polynomial_predict(c, points[i].x, points[i].y) - points[i].z;
	}
	return cumulative_sum / n;		
}